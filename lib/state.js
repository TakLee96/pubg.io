...
class Map { // refrehes per match
  ...
  movePlayer(playerId, direction) {
    if (this.players[playerId].geometry.canMove()) {
      this.players[playerId].geometry.lastMovement = Date.now();
      let { x, y } = direction;
  ...
  shootPlayer(playerId, direction) {
    if (this.players[playerId].geometry.canShoot()) {
      this.players[playerId].geometry.lastShot = Date.now();
  ...
  getActionNeighbors(playerId, action) {
...
class State { // persistents until server dies
  ...
  action(client, action) { // TODO: validate action
    ...
    switch (action.type) {
      case 'MOVE':
        success = this.map.movePlayer(playerId, action.direction);
        break;
      case 'SHOOT':
        success = this.map.shootPlayer(playerId, action.direction);
        break;
      default:
  ...
  getActionNeighbors(client, action) {
    return this.map.getActionNeighbors(client.id, action);
  }
}
